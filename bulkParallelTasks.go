...
func start(data string, events []wf.Event) ([]wf.Action, []wf.EventListener, error) {
	fmt.Printf("in start")
	return wf.PublishNext("compute", "task_1", "compute", "task_2", "compute", "task_3", "compute", "task_4", "compute", "task_5"),
		[]wf.EventListener{
			wf.NewEventListener("finalize", "some data", "task_1_done", "task_2_done", "task_3_done", "task_4_done", "task_5_done"),
		},
		nil
}

func compute(taskName string, events []wf.Event) ([]wf.Action, []wf.EventListener, error) {
